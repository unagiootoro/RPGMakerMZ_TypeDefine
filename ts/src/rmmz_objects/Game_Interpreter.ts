...
class Game_Interpreter {
    ...
    updateWaitMode() {
        let character = null;
        let waiting: boolean | null = false;
    ...
    // prettier-ignore
    operateValue(
        operation: number, operandType: number, operand: any
    ) {
    ...
    setupChoices(params: string | any[]) {
    ...
    command111(params: any[]) {
        ...
        switch (params[0]) {
            ...
            case 2: // Self Switch
                if (this._eventId > 0) {
                    const key: [number, number, string] = [this._mapId, this._eventId, params[1]];
    ...
    gameDataOperand(type: any, param1: number, param2: number) {
        let actor, enemy, character;
        switch (type) {
            case 0: // Item
    ...
    // Control Self Switch
    command123(params: any[]) {
        if (this._eventId > 0) {
            const key: [number, number, string] = [this._mapId, this._eventId, params[0]];
    ...
    // Change Battle BGM
    command132(params: any[]) {
        $gameSystem.setBattleBgm(params[0]);
        return true;
    }

    // Change Victory ME
    command133(params: any[]) {
        $gameSystem.setVictoryMe(params[0]);
        return true;
    }

    // Change Save Access
    command134(params: number[]) {
        if (params[0] === 0) {
    ...
    // Change Transparency
    command211(params: number[]) {
        $gamePlayer.setTransparent(params[0] === 0);
        return true;
    }

    // Show Animation
    command212(params: any[]) {
        this._characterId = params[0];
        const character = this.character(this._characterId);
        if (character) {
    ...
    // Wait
    command230(params: any[]) {
        this.wait(params[0]);
        return true;
    }

    // Show Picture
    command231(params: any[]) {
    ...
    // Rotate Picture
    command233(params: any[]) {
        $gameScreen.rotatePicture(params[0], params[1]);
        return true;
    }

    // Tint Picture
    command234(params: any[]) {
    ...
    // Erase Picture
    command235(params: any[]) {
        $gameScreen.erasePicture(params[0]);
        return true;
    }

    // Set Weather Effect
    command236(params: any[]) {
        if (!$gameParty.inBattle()) {
    ...
    // Play BGM
    command241(params: IAudioObject[]) {
        AudioManager.playBgm(params[0]);
        return true;
    }

    // Fadeout BGM
    command242(params: number[]) {
    ...
    // Play BGS
    command245(params: any[]) {
        AudioManager.playBgs(params[0]);
        return true;
    }

    // Fadeout BGS
    command246(params: number[]) {
        AudioManager.fadeOutBgs(params[0]);
        return true;
    }

    // Play ME
    command249(params: { name: any; }[]) {
        AudioManager.playMe(params[0]);
        return true;
    }

    // Play SE
    command250(params: { name: any; }[]) {
    ...
    // Change Tileset
    command282(params: any[]) {
        const tileset = $dataTilesets[params[0]];
        const allReady = tileset.tilesetNames
            .map((tilesetName: any) => ImageManager.loadTileset(tilesetName))
            .every((bitmap: { isReady: () => any; }) => bitmap.isReady());
        if (allReady) {
    ...
    command283(params: any[]) {
        $gameMap.changeBattleback(params[0], params[1]);
        return true;
    }

    // Change Parallax
    command284(params: any[]) {
        // prettier-ignore
    ...
    // Change HP
    command311(params: any[]) {
        const value = this.operateValue(params[2], params[3], params[4]);
        this.iterateActorEx(params[0], params[1], (actor: any) => {
    ...
    // Recover All
    command314(params: any[]) {
        this.iterateActorEx(params[0], params[1], (actor: { recoverAll: () => void; }) => {
            actor.recoverAll();
        });
        return true;
    }
    ...
    // Change Enemy HP
    command331(params: any[]) {
        const value = this.operateValue(params[1], params[2], params[3]);
        this.iterateEnemyIndex(params[0], (enemy: any) => {
    ...
    // Enemy Recover All
    command334(params: any[]) {
        this.iterateEnemyIndex(params[0], (enemy: { recoverAll: () => void; }) => {
            enemy.recoverAll();
        });
        return true;
    }
    ...
    // Enemy Appear
    command335(params: any[]) {
        this.iterateEnemyIndex(params[0], (enemy: { appear: () => void; }) => {
            enemy.appear();
    ...
    command337(params: any[]) {
        let param = params[0];
        if (params[2]) {
            param = -1;
        }
        const targets: any[] = [];
        this.iterateEnemyIndex(param, (enemy: { isAlive: () => any; }) => {
            if (enemy.isAlive()) {
                targets.push(enemy);
            }
        });
    ...
    // Force Action
    command339(params: any[]) {
        this.iterateBattler(params[0], params[1], (battler: Game_Battler) => {
    ...
    pluginCommand(command: any, args: any) {
        // deprecated
    }

    // Plugin Command
    command357(params: any[]) {
