...
class Bitmap {
    ...
    static snap(stage: { worldTransform: { identity: () => void; }; } | null) {
        ...
        const renderTexture = (PIXI.RenderTexture as any).create(width, height);
        if (stage) {
        ...
        (renderTexture as any).destroy({ destroyBase: true });
    ...
    resize(width: number, height: number) {
        ...
        (this.baseTexture as any).width = width;
        (this.baseTexture as any).height = height;
    }
    ...
    _createBaseTexture(source: any) {
        ...
        (this._baseTexture as any).height = source.height;
        this._updateScaleMode();
    }
