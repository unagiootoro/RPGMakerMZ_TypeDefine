 ...
 */
class _Window extends PIXI.Container {
    ...
    _refreshFrame() {
        const drect: IRectangle = { x: 0, y: 0, width: this._width, height: this._height };
        const srect: IRectangle = { x: 96, y: 0, width: 96, height: 96 };
        const m = 24;
    ...
    _refreshCursor() {
        const drect: IRectangle = this._cursorRect.clone();
        const srect: IRectangle = { x: 96, y: 96, width: 48, height: 48 };
        const m = 4;
    ...
    _setRectPartsGeometry(sprite: Sprite, srect: IRectangle, drect: IRectangle, m: number) {
        const sx = srect.x;
        const sy = srect.y;
        ...
        const dmw = dw - m * 2;
        const dmh = dh - m * 2;
        const children = sprite.children as unknown as Sprite[];
